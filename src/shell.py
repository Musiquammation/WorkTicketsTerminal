            ...
            while i < len(command_tokens):
                ...
                if tag:
                    if i + 1 >= len(command_tokens):
                        print(
                            f"Error: tag '{tag.name}' expects a value "
            ...
            for index, argument in enumerate(cmd.arguments):
                if index < len(positional_tokens):
                    ...
                    if argument.required:
                        print(
                            f"Error: missing required positional argument "
                            f"'{argument.name}'."
                        )
                        cmd.print_help()
                        return
...
def shell():
    ...
    print(
        "CLI started. Type 'help' to list commands or 'exit' to quit."
    )
